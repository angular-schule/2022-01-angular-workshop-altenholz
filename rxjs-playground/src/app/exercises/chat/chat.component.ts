...
import { Subject, ReplaySubject, merge, concat, race, forkJoin, EMPTY, map, startWith } from 'rxjs';
...
export class ChatComponent {
  ...
  constructor() {
    ...
    /**************!!**************/
     ...
     forkJoin([
       this.msg.julia$.pipe(startWith('')),
       this.msg.georg$.pipe(startWith('')),
       this.msg.john$.pipe(startWith('')),
     ]).subscribe({
      next: e => this.log(e),
